...
data_path = 'dataset/sick'
im_size = [512, 1024]
num_segments = 2
test_idx = get_sick_idx(data_path, train=False)

device = torch.device("cpu")
model = DeepLabv3Plus(models.resnet101(), num_classes=num_segments).to(device)
colormap = create_sick_label_colormap()

# visualise image id 61 in validation set
im_id = 5
im = Image.open('dataset/sick/images/val/{}.jpg'.format(im_id))
gt_label = Image.fromarray(sick_class_map(Image.open('dataset/sick/labels/val/{}.png'.format(im_id))))
im_tensor, label_tensor = transform(im, gt_label, None, crop_size=im_size, scale_size=(1.0, 1.0),augmentation=False)


model.load_state_dict(torch.load('model_weights/sick_label15_sup_0.pth', map_location=torch.device('cpu')))
model.eval()
...
plt.savefig("results/sick.jpg",dpi=600)